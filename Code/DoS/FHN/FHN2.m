...
% DoS
Dos_times = round(1 / Dos_h);
Dos_interval = round(rand(1, Dos_times) * N_t / Dos_times + (N_t / Dos_times) * (0:Dos_times-1));
k = Dos_interval(1);
Dos_flag = 0;
...
%     if ismember(it, Dos_interval)
%         k = round(rand(1) * (N_t / Dos_times) );
%         k = round(rand(1) * 6 * (N_t / Dos_times) );
%         k = floor(i/(N_t/Dos_times) + 1) * (N_t/Dos_times) - it;   
%         k = rand(1);
%         if (Dos_flag == 1)
%             Dos_flag = 0;
%         else
...
%     end
    ...
    for in = 1: N
        ...
        if (Dos_flag == 1)
            ...
            Y2(it+1, in) = Y2(it,in) + t_sample * (1*yy2(it,in) + 0.45*Y1(it,in) - 0.1*Y2(it,in));     
        else % Under control
            % Euler
            if (in*x_sample <= 0.5)
                v = 1;
